...
```
这个函数`first_word`，有一个 `&String` 类型的参数。我们不想要这个参数的所有权，同时也可以使用这个变量的数据，所以这样做很符合我们的需求。但是我们要返回什么？我们现在还没办法返回字符串的一个*部分*(因为还没学)。但是我们可以返回这个单词的最后一个字母所在的索引。就像代码4-7里面写的那样。
```rust
