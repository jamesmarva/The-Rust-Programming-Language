...
```
这有点像，我们引用了整个字符串，但是只提取出了[0...5]位(bit)。但是这不是引用了整个字符串，而是对字符长度部分的引用。
通过中括号的中的指定的范围 `[starting_index ... ending_index]`，我们可以创建一个切，`starting_index` 是切片的第一个位置，而 `ending_index` 要比切片的最后一个位置还要大 1。在内部，切片的数据结构会保存这个切片的在字符串的起始位置，以及这个切片的长度，这长度的数据是根据 `ending_index` 减去 `starting_index` 来得到的结果。因此在使用 `let word = &s[6..11]`后，world将会是一个切片，其中包含了一个指向了s的第7个字节的的指针，而且这个切片的长度为 5。
图 4-6 展现了这种状态：
